...
# and ServoCat as the telescope drive system.
...
def goto():
    ...
    if solved == False:
        ...
        return
    time.sleep(0.5)
    # Next block is fo ScopeDog
    #valid = nexus.get(":Sr" + goto_ra + "#")
    #valid = nexus.get(":Sd" + goto_dec + "#")
    #reply = nexus.get(":MS#")
    
    # Next block is for ServoCat, expand to send down USB
...
def stopSlew():
    print("g99.999 +99.999")
    # expand to send down USB
    servocat.send ("g99.999 +99.999")
    box_write("ServoCat stop", True)
...
def box_write(new_line, show_handpad):
    ...
    for i in range(0, 5, 1):
        tk.Label(window, text=box_list[i], bg=b_g, fg=f_g).place(x=1050, y=980 - i * 17)
...
def do_button(event):
    ...
    if button=='20':
        ...
        handpad.display('Up: Align/Sync','OK: Image+Solve','Dn: GoTo++')

def setTarget():
    print(gotoRa.get(),gotoDec.get())
    valid = nexus.get(":Sr" + gotoRa.get() + "#")
    time.sleep(0.05)
    gDec = gotoDec.get().split(':')
    if gDec[0][0] != "+" and gDec[0][0] != "-":
        gDec[0] = '+'+gDec[0] 
    gDec = gDec[0]+"*"+gDec[1]+":"+gDec[2]
    valid = nexus.get(":Sd" + gDec + "#")
